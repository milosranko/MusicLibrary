...
public class MusicBrainzService
{
	...
	public async Task<IEnumerable<MusicBrainzSearchResult>> Search(string artist, string release, string year)
	{
		...
		if (sb.Length > 0 && !string.IsNullOrWhiteSpace(release))
			sb.Append($"AND ");
		if (!string.IsNullOrWhiteSpace(release))
			sb.Append($"release:{release} ");
		if (sb.Length > 0 && !string.IsNullOrWhiteSpace(year))
			sb.Append("AND ");
	...
	{
		if (release.Equals(MusicBrainzSearchResult.Empty) || release.Id == Guid.Empty)
